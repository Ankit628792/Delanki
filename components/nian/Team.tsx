        ...
        <>
            <section className='min-h-screen w-full p-5 lg:p-10'>
                ...
                <div className='flex flex-wrap items-center justify-center gap-10 py-10'>
                    {
                        devs.map((dev: any) => <div className='w-full max-w-xs rounded-lg overflow-hidden border dark:border-gray-800 group relative max-h-[25rem] bg-white'>
                            ...
                            <div className='p-5 flex flex-col items-center justify-center gap-1 transform -translate-y-16'>
                                <div className='w-20 h-20 rounded-full relative cursor-pointer' onClick={() => Router.push(`/user/${dev._id}`)}>
                                    <Image className='object-cover rounded-full' src={dev.image} alt="" layout="fill" loading='lazy' />
                                </div>
                                <h1 className='text-xl font-semibold text-center cursor-pointer' onClick={() => Router.push(`/user/${dev._id}`)}>{dev.name}</h1>
                                <p className='text-gray-500 font-medium text-center cursor-pointer' onClick={() => Router.push(`/user/${dev._id}`)}>{dev.designation}</p>
